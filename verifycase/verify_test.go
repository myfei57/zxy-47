...
func TestGhTempFilterOrder(t *testing.T) {
	svc, _ := consoleServices(t)
	shedID := seed(t, svc, 8, 8)
	if err := svc.Fans.SetUpperBound(shedID, 32); err != nil {
		t.Fatal(err)
	}
	samples := []float64{28, 34, 28}
	if err := svc.Fans.Evaluate(shedID, samples); err != nil {
		t.Fatal(err)
	}
	fans, err := svc.Fans.Fans(shedID)
	if err != nil {
		t.Fatal(err)
	}
	for _, fan := range fans {
		if fan.Running {
			t.Fatalf("the fan must act on the filtered temperature, got running")
		}
	}
}
