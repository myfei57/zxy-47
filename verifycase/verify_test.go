...
func TestGhCurtainAfterSunPersist(t *testing.T) {
	svc, _ := consoleServices(t)
	shedID := seed(t, svc, 8, 8)
	zone := shedID + "-Z01"
	if _, err := svc.Sampler.SetReading(shedID, env.TypeSunlight, 900); err != nil {
		t.Fatal(err)
	}
	if err := svc.Sun.Record(shedID, 120); err != nil {
		t.Fatal(err)
	}
	if err := svc.Curtain.Retract(shedID, zone); err != nil {
		t.Fatal(err)
	}
	lamp, err := svc.Curtain.Lamp(shedID, zone)
	if err != nil {
		t.Fatal(err)
	}
	if lamp.On {
		t.Fatalf("backup lamp must follow the persisted sunlight, got on")
	}
	sun, err := svc.Sun.Read(shedID)
	if err != nil {
		t.Fatal(err)
	}
	if sun.Value != 900 {
		t.Fatalf("sunlight must be persisted before the curtain decision, got %v", sun.Value)
	}
}
