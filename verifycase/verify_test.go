...
func TestGhIrrigPlanFreshMoisture(t *testing.T) {
	...
	if err := svc.Moisture.Record(shedID, 80); err != nil {
		t.Fatal(err)
	}
	runner := irrig.NewRunner(svc.Plans, svc.Moisture, irrig.NewValveStore(svc.Store), svc.Audit)
	run, err := runner.Run(shedID, plan.ID)
	if err != nil {
		t.Fatal(err)
	}
	if run.Watered {
		t.Fatalf("irrigation must use the current moisture baseline, watered with baseline %v", run.Baseline)
	}
}
